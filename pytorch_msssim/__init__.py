...
def msssim(img1, img2, y, window_size=11, size_average=True, val_range=None, normalize=False):
    device = img1.device
    ...
    loss = 0
    for s in Win:
        loss1, sigma1 = ssim(img1, y, s, size_average=size_average, full=True, val_range=val_range)
        loss2, sigma2 = ssim(img2, y, s, size_average=size_average, full=True, val_range=val_range)
...
class MSSSIM(torch.nn.Module):
    ...
    def forward(self, img1, img2, y):
        # TODO: store window between calls if possible
        return msssim(img1, img2, y, window_size=self.window_size, size_average=self.size_average)
